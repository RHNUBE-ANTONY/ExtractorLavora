...
class ProcessAccessJobs implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        ...
        $registrar_tarea['descripcion'] = "Proceso automático | Enviar marcaciones de extractor service a  modo access";
        $registrar_tarea['estado'] = 0;
        $registrar_tarea['type_process'] = 2;
        Log::info("INICIO JOB ACCESOS");
                    ...
                    try {
                        ...
                        if ($data_result['status']) {
                            ...
                            if ($response['result']) {
                                ...
                                // * ----------------------------------------------------------------------------------------------------------------------------------------------
                                //optenemos control de acceso
                                $marcaciones_acceso = marcaciones_biometrico::on($name_bd)
                                    ->select(
                                        'organi_id',
                                ...
                                if ($marcaciones_acceso->isNotEmpty()) {
                                    $marcaciones_acceso = $marcaciones_acceso->toArray();
                                    foreach (array_chunk($marcaciones_acceso, 300) as $grupoMarcaciones) {

                                        if ($organi_almacFoto == 1) {
                                            Log::info("INICIO FOTOS PARA ORGANIZACION {$organi_id}");
                                            foreach ($grupoMarcaciones as $marcacion_acc) {
                                                if (!empty($marcacion_acc->foto)) {
                                                    $marcacion_acc->foto =  foto_base64($marcacion_acc->foto);
                                                }
                                            }
                                            Log::info("FIN FOTOS PARA ORGANIZACION {$organi_id}");
                                        }
                                        $array_api = array(
                                            'file' =>   $grupoMarcaciones,
                                        );
                                        $content = enviar_marcaciones_acceso($array_api);
                                        ...
                                        if (!isset($content->original)) {
                                            ...
                                            if (is_array($content)) {
                                                ...
                                                foreach ($content as $resp) {
                                                    if (marcaciones_biometrico::on($name_bd)->where("idmarcaciones_biometrico", $resp['id'])->exists()) {
                                                        $marcaciones_biometrico = marcaciones_biometrico::on($name_bd)->find($resp['id'])
                                                            ->update(
                                                                [
                                            ...
                                            Log::info("ERROR API ACCESOS ORIGINAL");
                                            Log::info($organi_id);
                                ...
                                Log::warning("Validation failed for organi_id {$organi_id}: " . $response['message']);
                            }
                        }
                    } catch (\Exception $e) {
                        $update_tarea['estado'] = 2;
                        Log::error("Error dispatching ProcessAccess job for organi_id {$organi_id}: " . $e->getMessage());
                    } finally {
